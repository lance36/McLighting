...
void setup() {
  ...
  // ***************************************************************************
    ...
    if (mqtt_host != "" && String(mqtt_port).toInt() > 0) {
      snprintf(mqtt_intopic, sizeof mqtt_intopic, "home/%s/in", HOSTNAME);
      snprintf(mqtt_outtopic, sizeof mqtt_outtopic, "home/%s/out", HOSTNAME);
  ...
  server.on("/off", []() { exit_func = true; mode = OFF; getArgs();
  getStatusJSON(); });
